...
bool SlidingWindowFlow::InitCalibration()
{
...
bool SlidingWindowFlow::ValidData() {
    ...
    current_laser_odom_data_.pose.block<3, 3>(0, 0) = tmp.transpose() * current_laser_odom_data_.pose.block<3, 3>(0, 0);
...
bool SlidingWindowFlow::UpdateIMUPreIntegration(void) {
    while (
        ...
        if (!sliding_window_ptr_->UpdateIMUPreIntegration(current_imu_raw_data_))
        {
            break;
        }
...
bool SlidingWindowFlow::PublishData() {
    if ( sliding_window_ptr_->HasNewKeyFrame() ) {
        ...
        // Eigen::Matrix3f tmp;
        // tmp << 0, -1, 0, 1, 0, 0, 0, 0, 1;
        // key_frame.pose.block<3, 3>(0, 0) = tmp * key_frame.pose.block<3, 3>(0, 0);
        // // _________________________________________ //
