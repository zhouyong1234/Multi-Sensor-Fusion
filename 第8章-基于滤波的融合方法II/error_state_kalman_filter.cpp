...
void ErrorStateKalmanFilter::UpdateOdomEstimation(
  ...
  GetAngularDelta(index_curr_, index_prev_, angular_delta, angular_vel_mid);
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
    const Eigen::Matrix4d &T_nb, const Eigen::Vector3d &v_b, Eigen::Matrix<double, 9, 1> &Y,Eigen::Matrix<double, 9, 15> &G, Eigen::Matrix<double, 15, 9> &K) {
  ...
  YPose_.block<3,1>(3,0) = dv;
  YPose_.block<3,1>(6,0) = dtheta;
  Y = YPose_;
  GPose_.setZero();
  GPose_.block<3,3>(0, kIndexErrorPos) = Eigen::Matrix3d::Identity();
  GPose_.block<3,3>(3, kIndexErrorVel) = pose_.block<3,3>(0,0).transpose(); 
  GPose_.block<3,3>(3, kIndexErrorOri) = Sophus::SO3d::hat(v_b);
  // GPose_.block<3,3>(3, kIndexErrorOri) = Sophus::SO3d::hat(pose_.block<3,3>(0,0).transpose()*vel_);
  GPose_.block<3,3>(6, kIndexErrorOri) = Eigen::Matrix3d::Identity();
  ...
  CPose_.block<3,3>(6,kIndexNoiseBiasAccel) = Eigen::Matrix3d::Identity();

  Eigen::MatrixXd C = CPose_;

  // TODO: set Kalman gain:             
  Eigen::MatrixXd R = RPose_;
